...
class ModernGUI:
    ...
    def create_evaluation_tab(self, tab):
        ...
        agent_frame_eval.grid(row=1, column=0, columnspan=3, sticky="ew", padx=20, pady=5)
        ctk.CTkLabel(agent_frame_eval, text="Agent to Evaluate:").grid(row=0, column=0, sticky="w")
        self.agent_to_evaluate = ctk.StringVar(value="dqn")
        agent_menu_eval = ctk.CTkOptionMenu(agent_frame_eval, variable=self.agent_to_evaluate, values=["dqn", "d3qn", "q-learning"], corner_radius=6)
    ...
    def run_command_in_thread(self, command):
        def target():
            ...
            try:
                ...
                self.log("Please ensure you have a '.venv' folder with Python executable.\n")
    ...
    def run_training(self):
        ...
        output_path = filedialog.asksaveasfilename(
            ...
            initialdir=os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "models"),
            defaultextension=".pth",
            filetypes=[("PyTorch Model", "*.pth"), ("Pickle file", "*.pkl"), ("All files", "*.*")]
        )
        if not output_path:
        ...
        if not net_file or not route_file:
            messagebox.showerror("Error", "Please specify Network and Routes files in the 'SUMO Setup' tab.")
            return
    ...
    def run_evaluation(self):
        ...
        if scenario == 'A': # Rich Data
            ...
            if not eval_data:
                messagebox.showerror("Error", "For Scenario A, please provide the evaluation data file.")
            ...
            if not aadt.isdigit() or int(aadt) <= 0:
                messagebox.showerror("Error", "For Scenario B, please provide a valid AADT value.")
