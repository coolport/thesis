...
def main():
    ...
    for metric, goal in metrics.items():
        if goal == 'min':
            ...
            percent_diff = (df[metric] - best_value) / best_value if best_value != 0 else 0
        else: # max
            best_value = df[metric].max()
            percent_diff = (best_value - df[metric]) / best_value if best_value != 0 else 0
        ranks[f'{metric}_rank'] = np.maximum(0, 10 - (percent_diff * 10))
    ...
    trials = {
        "Trial 1: Balanced": {'avg_wait_time': 0.33, 'avg_queue_length': 0.33, 'total_throughput': 0.34},
        "Trial 2: Prioritize Flow": {'avg_wait_time': 0.15, 'avg_queue_length': 0.15, 'total_throughput': 0.70},
        "Trial 3: Prioritize Wait Time": {'avg_wait_time': 0.80, 'avg_queue_length': 0.10, 'total_throughput': 0.10},
    }
    sensitivity_results = pd.DataFrame(index=df.index)

    for trial_name, weights in trials.items():
        for agent in df.index:
            wcs_wait = ranks.loc[agent, 'avg_wait_time_rank'] * weights['avg_wait_time']
            wcs_queue = ranks.loc[agent, 'avg_queue_length_rank'] * weights['avg_queue_length']
            wcs_throughput = ranks.loc[agent, 'total_throughput_rank'] * weights['total_throughput']
            total_score = wcs_wait + wcs_queue + wcs_throughput
    ...
    print("Analysis complete. Report saved to analysis_report.html")
